...
def sample_using_diffusion(
    ...
    z = torch.cat([z, mri_image_encode], dim=1)
    ...
    for t in progress_bar:
        with torch.no_grad():
            with autocast(enabled=True):
                ...
                x=z[:, :3, :, :, :]-noise_pred
                z= torch.cat([x, mri_image_encode], dim=1)
                noise_pred2 = torch.cat([noise_pred, torch.zeros_like(mri_image_encode)], dim=1)
                # z, _ = scheduler.step(noise_pred, t, z)
                z, _ = scheduler.step(noise_pred2, t, z)
                z = torch.cat([z[:, :3, :, :, :], mri_image_encode], dim=1)
