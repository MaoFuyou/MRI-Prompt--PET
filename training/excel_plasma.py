...
def process_pet_data(data_PET, base_path, output_csv, output_folder,plasma_folder):
    """
    ...
    """
    filtered_data = []
    PET_plasma = pd.read_csv(plasma_folder)
    ...
    for idx, row in data_PET.iterrows():
        ...
        Sex = row['Sex']
        Age = row['Age']
        matched_row = PET_plasma[PET_plasma['PTID'] == subject_id]

        if matched_row.empty or 'TESTVALUE' not in matched_row.columns:
            continue

        test_value = matched_row['TESTVALUE'].values[0]
...
def process_mri_data(data_MRI, base_path, output_csv, output_folder, plasma_folder):
    """
    Process PET data to filter and convert images.
        ...
        if matching_folders:
            try:
                for i in range(len(matching_folders)):
                    ...
                    output_filename = f"{subject_id}_{test_value}_{study_date}_{matching_id[i][0]}_MRI.nii.gz"##MMSE
                    convert_dcm_to_nii(image_folder, output_folder, output_filename)
...
if __name__ == '__main__':
    ...
    filtered_df = process_pet_data(data_PET, base_path, output_csv, output_folder, plasma_folder)
    ...
    filtered_df = process_mri_data(data_MRI, base_path, output_csv, output_folder, plasma_folder)
    filtered_df.to_csv(output_csv, index=False)
    print(f"Filtered data saved successfully to {output_csv}!")
